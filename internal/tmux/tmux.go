...
// pickerScript is the prefix-s session switcher: sesh lists (tmux
// sessions, zoxide-ranked dirs, sesh configs), fzf chooses, sesh
// connects. A script rather than a conf one-liner so the quoting has a
// home and Esc quietly closes the popup.
const pickerScript = `#!/bin/sh
# Generated by rook — do not edit; edits are overwritten on every boot.
selected="$(sesh list --icons | fzf --ansi --no-sort --reverse \
  --prompt '♜ ' \
  --header 'C-a all · C-t sessions · C-x dirs · C-d kill' \
  --bind 'ctrl-a:change-prompt(♜ )+reload(sesh list --icons)' \
  --bind 'ctrl-t:change-prompt(♜ session )+reload(sesh list -t --icons)' \
  --bind 'ctrl-x:change-prompt(♜ dir )+reload(sesh list -z --icons)' \
  --bind 'ctrl-d:execute(tmux kill-session -t {2..})+reload(sesh list -t --icons)')" || exit 0
...
func WriteConf(s Settings) (string, error) {
	...
	if err := writeAtomic(pickerPath(path), pickerScript, 0o755); err != nil {
		return "", err
	}
