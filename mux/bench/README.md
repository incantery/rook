...
`./run.py [runs]` — rook's engine vs tmux on the same outer glass:
throughput (wall time for a 5MB cat to reach the screen), server RSS,
and the engine's own input→frame latency histogram.

2026-08-22, M-series MacBook, ReleaseSafe:

    rook drain: median 0.338s        tmux: 0.489s   (0.69x)
    rook RSS:   6.9 MB               tmux: 11.1 MB
    rook input→frame: p50 52µs · p99 81µs
