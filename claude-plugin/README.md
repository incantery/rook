...
line ("Asked in rook →"), the blocked `mcp__rook__ask` call waits for the
answer, and the human decides with j/k · 1-9 · Enter (Esc dismisses).

Two parts, both inert outside rook:

- **`mcpServers.rook`** — `rookctl mcp`, a stdio server exposing the `ask`
  tool (`rookctl ask` behind a tools/call). Needs `rookctl` on PATH
  (`make install` puts it there) and `$ROOK_SESSION` in the environment,
  which every rook pty exports.
- **`hooks/ask-redirect.sh`** — a PreToolUse hook that denies the built-in
  AskUserQuestion when `$ROOK_SESSION` is set, steering claude to the MCP
  tool so the question is asked once, in one place.
